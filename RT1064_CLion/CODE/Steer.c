...
void Steer_PIDStruct_Init(PID_Struct Steer_PID, Filter_Struct Steer_Filter) {
    ...
    Steer_PID->KP = 13.0f;
    Steer_PID->KI = 0;
    Steer_PID->KD = 1.5f;//1.32;
    Steer_PID->D_LastError = 0;
    Steer_PID->D_LastError2 = 0;
...
void SteerCtrl(PID_Struct Steer_PID, Filter_Struct Steer_Filter) {
    // if (Camera == Find_Trident && TridentFlag == 0) {
    ...
    if (Steer_PID->CurrentValue >= 2.5) {
        Steer_PID->KP = 13.0f + (FastABS(Steer_PID->CurrentValue) / 10.0f) * 8.0f;
    }
